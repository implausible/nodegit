...
const {
  ...
  reset: static_Reset,
  fromAnnotated: static_FromAnnotated
} = Reset;
...
Reset.default = (repo, target, pathspecs) => {
  return static_Default(repo, target, pathspecs);
};
 ...
 *
 * @param {CheckoutOptions} options Checkout options to be used for a HARD reset.
 ...
 * @return {Number} 0 on success or an error code
 */
Reset.reset = (repo, target, resetType, options) => {
  if (repo !== target.repo) {
  ...
  const normalizedOptions = normalizeOptions(options, CheckoutOptions);
  return static_Reset(repo, target, resetType, normalizedOptions);
};
 ...
 */
Reset.fromAnnotated = (repo, target, resetType, options) => {
  const normalizedOptions = normalizeOptions(options, CheckoutOptions);
  return static_FromAnnotated(repo, target, resetType, normalizedOptions);
};
