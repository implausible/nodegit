...
Repository.prototype.deleteTagByName = function(name) {
  return Tag.delete(
    this,
    ~name.indexOf('refs/tags/') // eslint-disable-line no-bitwise
      ? name.substr(10)
      : name,
  );
};
...
Repository.prototype.getStatus = async function(options) {
  ...
  const statusFiles = [];
  await Status.foreachExt(this, defaultedOptions, (statusPath, status) => {
    statusFiles.push(new StatusFile({ path: statusPath, status }));
  });
...
Repository.prototype.rebaseBranches = async function(
  ...
  if (maybeUpstreamAnnotatedCommit) {
    ...
    if (oid.toString() === maybeUpstreamAnnotatedCommit.id().toString()) {
...
Repository.prototype.mergeBranches = async function(
  ...
  if (mergePreference !== Merge.PREFERENCE.FASTFORWARD_ONLY) {
  ...
  const filePathsAndShouldFilter = await Promise.all(filePaths.map(
    async (_filePath) => {
      ...
      return {
        path: _filePath,
        filter: Boolean(
          (status & Status.STATUS.WT_MODIFIED) // eslint-disable-line no-bitwise
          || (status & Status.STATUS.INDEX_MODIFIED), // eslint-disable-line no-bitwise
        ),
      };
    },
  ));
