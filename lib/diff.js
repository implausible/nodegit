...
const makeLcs = (diffByFileWeakMap) => (file1, file2) => {
  const diffPoints = diffByFileWeakMap[file2];
  const lcsChain = [];
  ...
  const firstDiffPoint = diffPoints[0];
  if (firstDiffPoint && firstDiffPoint.origin === ' ' && firstDiffPoint.oldLineNumber !== 1) {
    for (let i = 0; i < firstDiffPoint.oldLineNumber - 1; ++i) {
      const thisPositionIndex = lcsChain.length;
      lcsChain.push({
        file1Index: i,
        file2Index: i,
        get chain() {
          return lcsChain[thisPositionIndex + 1];
        }
      });
    }
  }
  ...
  for (const diffPoint of diffPoints) {
    if (diffPoint.origin === ' ') {
      // check last common position, if it is not adjacent, backfill up to this point
      const file1Index = diffPoint.oldLineNumber - 1;
      const file2Index = diffPoint.newLineNumber - 1;

      const lastLcs = lcsChain[lcsChain.length - 1];

      if (lastLcs && lastLcs.file1Index + 1 !== file1Index && lastLcs.file2Index !== file2Index) {
        console.log(lastLcs);
        console.log('case 2');
        for (let f1i = lastLcs.file1Index + 1, f2i = lastLcs.file2Index; f1i < file1Index; ++f1i, ++f2i) {
          const thisPositionIndex = lcsChain.length;
          lcsChain.push({
            file1Index: f1i,
            file2Index: f2i,
  ...
  const lastDiffPoint = diffPoints[diffPoints.length - 1]
  if (lastDiffPoint && lastDiffPoint.origin === ' ') {
    console.log('lastDiff');
    for (
      let file1Index = lastDiffPoint.oldLineNumber - 1, file2Index = lastDiffPoint.newLineNumber - 1;
      file1Index < file1.length;
