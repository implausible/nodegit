 ...
 * @return {Tree}
 */
TreeEntry.prototype.getTree = async function() {
  const tree = await this.parent.repo.getTree(this.id());
  tree.entry = this;
  return tree;
};
...
TreeEntry.prototype.path = function() {
  return path.join(this.parent.path(), this.dirtoparent || '', this.name());
};
